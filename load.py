...
def generate_dataset_from_images(just_Y=False):
    ...
    np.save(os.path.join(datasets_dir, lfw_image_saples), X)
...
def lfw(num_train=10000, onehot=True):
    X = np.load(os.path.join(datasets_dir, lfw_image_samples))
    Y = np.load(os.path.join(datasets_dir, lfw_image_labels))
